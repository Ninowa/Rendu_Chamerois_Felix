...
{ 
    ...
    {
        if (Input.GetButtonDown ("center_camera")){

            rotateBehind=!rotateBehind;
        }
    
        if (Input.GetAxis ("Horizontal_R") || Input.GetAxis ("Vertical_R") || Input.GetMouseButton(0) || Input.GetMouseButton(1)) 
       { 
            //Check to see if mouse input is allowed on the axis
            if (allowMouseInputX)
                xDeg += Input.GetAxis ("Horizontal_R") * xSpeed * 0.02; 
              // xDeg += Input.GetAxis ("Mouse X") * xSpeed * 0.02; 
            else
                RotateBehindTarget();
            if (allowMouseInputY)
                yDeg -= Input.GetAxis ("Vertical_R") * ySpeed * -0.02; 
